...
def openrouter_fallback_parse(text: str) -> List[Dict]:
    key = st.secrets["OPENROUTER_API_KEY"]
    if not key:
        st.warning("OpenRouter API key not set. Skipping LLM fallback.")
        return []
    ...
    prompt = f"""
Extract course data from this transcript text. Return a JSON array. Each item should have:
- course_code
- title
- credits
- grade
...
st.text_area("🧠 LLM Raw Output (Debug)", content, height=200)
...
try:
    ...
    return []
        ...
        st.error(f"OpenRouter fallback failed: {e}")
        return []
...
if uploaded_file:
    if uploaded_file.name.endswith(".csv"):
        ...
        if not data:
            st.warning("No structured data found with regex. Attempting OpenRouter fallback...")
            parsed = openrouter_fallback_parse(text)
            for item in parsed:
                try:
                    data.append({
