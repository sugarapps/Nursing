...
from openai import OpenAI  # NEW: For OpenRouter
...
def get_course_description_from_web(course_row):
    ...
    fallback_message = f"Auto-search couldn't find a description. For a more accurate comparison, [click to search Google]({search_url}) and paste the result below."
    try:
...
# Session state and sidebar remain the same
...
# Data processing loop remains the same
if uploaded_files:
    all_courses_list = []
        ...
        for req_name, req_details in UA_PREREQS.items():
            with st.expander(f"**{req_name}** ({req_details['ua_course']})", expanded=False):
                ...
                if selected_course_code != '- Not Selected -':
                    ...
                    # --- NEW: Call OpenRouter and display rich results ---
    ...
    # The Final Summary Report tab (tab2) logic remains unchanged
    with tab2:
