...
def get_course_description_from_web(course_row):
    """
    A best-effort web scraper that uses DuckDuckGo and provides a fallback search link.
    """
    university = course_row['University']
    course_code = course_row['Course_Code']
    course_title = course_row['Title']

    # --- FIX: Create a more specific search query including the title ---
    ...
    # Use the HTML version of DDG which is simpler to parse
    html_search_url = "https://html.duckduckgo.com/html/"
    params = {"q": query}
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36'}
    
    # --- FIX: Create a helpful fallback message with a clickable link ---
    fallback_message = f"Search failed. [Try this manual search link]({search_url}) and paste the description."

    try:
        response = requests.post(html_search_url, data=params, headers=headers, timeout=10)
        response.raise_for_status()
        soup = BeautifulSoup(response.text, 'html.parser')
        
        search_results = soup.find_all('div', class_='result')
        if not search_results: return fallback_message
        ...
        if match_pos != -1:
            ...
            return description + "..."
        return f"Found a page, but couldn't auto-extract a description. [Try this manual search link]({search_url}) and paste the result."
    except Exception:
...
if uploaded_files:
    ...
    if not all_courses_list:
        ...
        st.info("The table below shows all courses parsed. The app attempts to auto-detect the university for each transcript. **You can edit any cell, including the University.**")
        edited_df = st.data_editor(
            master_df, num_rows="dynamic", use_container_width=True,
            column_config={ "University": st.column_config.TextColumn("University", help="The university where the course was taken. Auto-detected, but can be edited."), "Grade_Points": st.column_config.NumberColumn("Grade Points", format="%.1f"), "Source": st.column_config.TextColumn("Source File") },
        ...
        for req_name, req_details in UA_PREREQS.items():
            with st.expander(f"**{req_name}** ({req_details['ua_course']})", expanded=False):
                ...
                if selected_course_code != '- Not Selected -':
                    ...
                    with col1:
                        st.info(f"**U of A Prerequisite Context**")
                        st.write(ua_context)
                    ...
                    with col2:
                        ...
                        # --- FIX: Intelligently handle the text box based on search result ---
                        description_to_show = ""
                        if "Search failed" in scraped_desc or "couldn't auto-extract" in scraped_desc:
                            st.warning(scraped_desc, icon="⚠️") # Show the error/link to the user
                        else:
                            description_to_show = scraped_desc # Pre-fill the box with the successful result
                        
                        manual_desc = st.text_area(
                            "Verify or paste the description here for the most accurate comparison:", 
                            value=description_to_show,
                            key=f"desc_{req_details['ua_course']}", height=150
                        )
                        
                        student_context_full = f"{base_student_context}. {manual_desc}"

                    similarity_score = calculate_similarity(ua_context, student_context_full)
    ...
    with tab2:
        # GPA and Summary Report logic remains the same
