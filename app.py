...
def get_course_description_from_web(university, course_code):
    """
    A more robust attempt to scrape a course description from a Google search.
    """
    try:
        query = f'"{university}" "{course_code}" course catalog description'
        search_url = f"https://www.google.com/search?q={query}"
        
        # --- FIX: Use a realistic User-Agent to mimic a real browser ---
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36'
        }
        
        # st.write(f"_Searching for: {query}..._") # Optional: uncomment for debugging
        response = requests.get(search_url, headers=headers, timeout=10)
        response.raise_for_status()
        
        soup = BeautifulSoup(response.text, 'html.parser')
        
        # --- FIX: Find search result containers instead of specific URL patterns ---
        ...
        if not search_results:
            if "Our systems have detected unusual traffic" in response.text:
                return "Web search failed: Google is blocking automated requests from this server (CAPTCHA)."
            return "Could not find any search results on the page. The page structure may have changed."
        ...
        match_pos = page_text_no_space.lower().find(course_code_no_space.lower())
        
        if match_pos != -1:
            start_index = max(0, page_text.lower().find(course_code.lower()) - 50)
    ...
    with tab2:
        ...
        with col2:
            ...
            if not courses_for_gpa.empty:
                ...
                total_quality_points = (pd.to_numeric(full_course_data['Grade_Points']).fillna(0)).sum()
