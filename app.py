...
def get_course_description_from_web(course_row):
    """
    A best-effort web scraper that uses Google and provides a fallback search link.
    """
    university = course_row['University']
    course_code = course_row['Course_Code']
    course_title = course_row['Title']

    # --- FIX: Create a specific search query for Google ---
    query = f'"{university}" "{course_code}" "{course_title}" course description'
    search_url = f"https://www.google.com/search?q={requests.utils.quote(query)}"
    
    # --- FIX: Create a helpful fallback message with a clickable Google link ---
    fallback_message = f"Search failed. [Try this Google search link]({search_url}) and paste the description."
    ...
    try:
        ...
        # --- FIX: Use common Google class names for search results ---
        ...
        if not search_results:
            if "Our systems have detected unusual traffic" in response.text:
                return f"Search blocked by Google (CAPTCHA). [Try this manual search link]({search_url}) and paste."
            ...
            with st.expander(f"**{req_name}** ({req_details['ua_course']})", expanded=False):
                ...
                if selected_course_code != '- Not Selected -':
                    ...
                    with col2:
                        ...
                        description_to_show = ""
                        if "Search failed" in scraped_desc or "couldn't auto-extract" in scraped_desc or "Search blocked" in scraped_desc:
                            st.warning(scraped_desc, icon="⚠️")
                        else:
