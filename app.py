...
def get_course_description_from_web(university, course_code):
    """
    A more reliable web scraper that uses DuckDuckGo instead of Google.
    """
    try:
        query = f'"{university}" "{course_code}" course catalog description'
        
        # --- FIX: Use DuckDuckGo which is more scraper-friendly ---
        ...
        # --- FIX: Look for DuckDuckGo's result structure ---
        search_results = soup.find_all('div', class_='result')
        if not search_results:
            return "Could not find any search results on DuckDuckGo. The page structure may have changed."
        ...
        if not page_url:
            return "Found search result blocks, but could not extract a link."

        # Visit the found page to get the description
        ...
        # Heuristic to find the description near the course code
        course_code_no_space = course_code.replace(" ", "")
        page_text_lower = page_text.lower()
        course_code_lower = course_code.lower()
        
        match_pos = page_text_lower.find(course_code_lower)
        if match_pos == -1:
            # Try again without spaces
            match_pos = page_text_lower.replace(" ", "").find(course_code_no_space.lower())

        if match_pos != -1:
            start_index = max(0, match_pos - 100) # Look a bit before the course code
            end_index = start_index + 600
            description = ' '.join(page_text[start_index:end_index].split())
            return description + "..."
            
        return "Found the course page, but could not automatically extract a description. Please copy/paste it."
        
    except requests.exceptions.HTTPError as http_err:
        return f"Web search failed: HTTP Error {http_err.response.status_code}. The website may be down or blocking requests."
    except Exception as e:
        return f"An unexpected error occurred during web scraping: {e}. Please find and paste the description manually."
    ...
    with tab2:
        ...
        with col2:
            ...
            if not courses_for_gpa.empty:
                ...
                # Ensure Grade_Points is numeric and fill NaNs with 0 before summing
