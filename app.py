...
def detect_university_from_text(text):
    """
    A best-effort attempt to find a university name in the first part of the transcript text.
    """
    # Look at the first 500 characters, which usually contain the header
    text_header = text[:500]
    
    # Regex to find lines containing "University", "College", "Institute", etc.
    # It captures the full line for context.
    ...
    if matches:
        # Return the first full line that matches, stripped of whitespace.
        # e.g., "The Official University of Somewhere"
...
with st.sidebar:
    ...
    # --- REMOVED: University name input is now automated ---
...
if uploaded_files:
    ...
    with st.spinner("Processing uploaded transcripts..."):
        for uploaded_file in uploaded_files:
            ...
            if raw_text:
                # --- NEW: Auto-detect university from each file ---
                ...
                if not parsed_df.empty:
                    ...
                    parsed_df['University'] = detected_university # Assign detected name
    ...
    if not all_courses_list:
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                "University": st.column_config.TextColumn("University", help="The university where the course was taken. Auto-detected, but can be edited."),
        ...
        for req_name, req_details in UA_PREREQS.items():
            with st.expander(f"**{req_name}** ({req_details['ua_course']})", expanded=False):
                ...
                if selected_course_code != '- Not Selected -':
                    ...
                    # --- REVISED: No more web scraping. Direct AI comparison of known info. ---
                    col1, col2 = st.columns(2)
                    
                    # Create rich context strings for comparison
                    student_context = f"{student_course['University']} {student_course['Course_Code']} {student_course['Title']}"
                    ...
                    with col2:
                        ...
                        st.write(student_context)
                    
                    # AI compares the two rich context strings
                    similarity_score = calculate_similarity(ua_context, student_context)
    ...
    with tab2:
        # This tab's logic remains largely the same and works with the new data structure.
