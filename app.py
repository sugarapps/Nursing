...
# Expanded to handle common non-grade characters found in transcripts
...
def parse_nau_transfer_format(text):
    # This parser looks for blocks starting with "Incoming Course"
    all_courses = []
    # Split the entire text by the "Incoming Course" delimiter
    course_blocks = text.split("Incoming Course")
    if len(course_blocks) < 2: return pd.DataFrame() # No "Incoming Course" found
    
    # Regex to find the course line, which is usually right after the delimiter
    pattern = re.compile(r'^\s*([A-Z]{2,4})\s+(\d{3,4}[A-Z]*)\s+(.*?)\s+(\d{1,2}\.\d{2})\s+([A-Z][+-]?)', re.MULTILINE)
    
    for block in course_blocks[1:]: # Skip the first chunk before any delimiter
        match = pattern.search(block)
        if match:
            subject, number, title, credits, grade = match.groups()
            course_code = f"{subject} {number}"
            all_courses.append([course_code, title.strip(), credits, grade])
...
def parse_courses_from_text(text):
    """
    Master parser that tries multiple parsing strategies in order of specificity.
    """
    parsers = [
        parse_pima_format,
        parse_maricopa_format,
        parse_nau_transfer_format
    ]
    
    for parser_func in parsers:
        df = parser_func(text)
        if not df.empty:
            # Shared cleanup for any successful parser
    ...
    # If all parsers fail, return an empty DataFrame
    return pd.DataFrame()
...
if uploaded_files:
    all_courses_list = []
    # --- UI now shows raw OCR for debugging ---
    with st.expander("Show Raw OCR Text from Transcripts"):
        for uploaded_file in uploaded_files:
            st.subheader(f"OCR Text for: {uploaded_file.name}")
            raw_text = extract_text_from_file(uploaded_file)
            st.text_area(f"text_{uploaded_file.name}", raw_text, height=200, disabled=True)
            if raw_text:
                detected_university = detect_university_from_text(raw_text)
                parsed_df = parse_courses_from_text(raw_text) # Use the new master parser
    ...
    if not all_courses_list:
        st.error("Automated parsing failed for all documents. This may be due to a new or unsupported transcript format. Please use the manual entry feature below.")
        ...
        st.info("The table below shows all successfully parsed courses. **You can edit any cell, including the University.**")
    ...
    st.divider()
    # The rest of the UI tabs remain the same and will function correctly with the data
