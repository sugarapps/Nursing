            ...
            for req_name, req_details in UA_PREREQS.items():
                with st.expander(f"**{req_name}** ({req_details['ua_course']})", expanded=False):
                    ...
                    if selected_course_code != '- Not Selected -':
                        ...
                        # Calculate the raw similarity score
                        similarity_score = calculate_similarity(req_details['description'], manual_desc)
                        
                        # --- FIX: CLAMP THE VALUE FOR THE PROGRESS BAR ---
                        # The progress bar widget requires a value between 0.0 and 1.0.
                        # We'll treat any negative (dissimilar) score as 0 for the visual bar.
                        progress_value = max(0.0, min(1.0, similarity_score))

                        # Display the progress bar with the clamped value,
                        # but show the TRUE score in the text for the advisor.
        ...
        with tab2:
            ...
            with col2:
                ...
                selected_courses_df = st.data_editor(
                    ...
                    disabled=['Course_Code', 'Title', 'Credits', 'Grade']
                )
