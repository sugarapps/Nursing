...
def load_ai_model():
    ...
    # Use the smaller model to be safe on Streamlit Community Cloud
    return SentenceTransformer('paraphrase-MiniLM-L3-v2')

# Use --find-links in requirements.txt for torch to ensure CPU-only version
model = load_ai_model()
...
UA_PREREQS = {
    ...
    "Microbiology w/ Lab": {
        "ua_course": "MIC 205A/L", "credits": 4,
    ...
    "Chemistry w/ Lab": {
        "ua_course": "CHEM 151 or higher", "credits": 4,
    ...
    "Statistics": {
        "ua_course": "MATH 163 or higher", "credits": 3,
...
def parse_courses_from_text(text):
    # Regex to find course code, title, credits, and grade. This is a best-effort.
...
if uploaded_file and university_name:
    ...
    if raw_text:
        ...
        edited_df = st.data_editor(
            parsed_df, num_rows="dynamic", use_container_width=True,
            column_config={"Grade_Points": st.column_config.NumberColumn("Grade Points", format="%.1f")},
            key="course_editor"
        )
        ...
        with tab1:
            # This tab's logic remains the same
        ...
        with tab2:
            ...
            # --- NEW: Last 60 Credits GPA with Manual Selection ---
            with col2:
                ...
                # Create a copy of the dataframe to add a 'Select' column
                selection_df = edited_df.copy()
                selection_df['Select'] = False # Default to not selected
                
                # Reorder for better UI
                selection_df = selection_df[['Select', 'Course_Code', 'Title', 'Credits', 'Grade']]
                ...
                selected_courses_df = st.data_editor(
                    ...
                    hide_index=True,
                    disabled=['Course_Code', 'Title', 'Credits', 'Grade'] # Make data columns non-editable here
                )

                # Filter to only the rows the user has checked
                courses_for_gpa = selected_courses_df[selected_courses_df['Select']]

                if not courses_for_gpa.empty:
                    # Get the full data for the selected courses from the original dataframe
                    full_course_data = edited_df[edited_df['Course_Code'].isin(courses_for_gpa['Course_Code'])]
