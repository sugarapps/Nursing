...
def get_course_description_from_web(course_row):
    ...
    # --- FIX: Reword the fallback message to be more constructive ---
    ...
    try:
        ...
        if not search_results:
            if "Our systems have detected unusual traffic" in response.text:
                return f"Auto-search was blocked by Google (CAPTCHA). [Please click here to run the search manually]({search_url}) and paste the result."
        ...
        if match_pos == -1:
            match_pos = page_text.lower().replace(" ", "").find(course_code.lower().replace(" ",""))
...
if uploaded_files:
    ...
    with tab1:
        ...
        # --- FIX: Create a lookup for the format_func to make selectbox more readable ---
        # Ensure we don't have duplicate course codes which would break the dictionary
        ...
        for req_name, req_details in UA_PREREQS.items():
            with st.expander(f"**{req_name}** ({req_details['ua_course']})", expanded=False):
                ...
                # Use the list of unique course codes for the options
                ...
                selected_course_code = st.selectbox(
                    ...
                    index=current_index, 
                    key=select_key,
                    # --- FIX: Display Code and Title, but return only the Code ---
                    format_func=lambda code: f"{code} - {course_code_to_title.get(code, 'Unknown Title')}" if code != '- Not Selected -' else '- Not Selected -'
                )
                ...
                if selected_course_code != '- Not Selected -':
                    ...
                    with col2:
                        ...
                        description_to_show = ""
                        # --- FIX: Display a more positive message on failure ---
                        if "Auto-search" in scraped_desc or "couldn't auto-extract" in scraped_desc:
                            st.info(scraped_desc, icon="💡")
                        else:
                            description_to_show = scraped_desc
